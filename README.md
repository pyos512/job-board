...
> 매일 자동 갱신은 이미 **작업 스케줄러에 등록되어 매일 오전 8시·오후 5시** 실행됩니다(아래 참고).
...
작업 스케줄러(`PublicJobBoard_DailyUpdate`, 매일 08:00·17:00)가 `run_daily.bat` → **`publish.py`** 를 실행:
...
```
- 로그: `logs\daily.log`.
- 📌 **절전(잠자기) 중에도 매일 08시·17시에 스스로 깨어나 실행 후 다시 잠듦** — 작업의 `WakeToRun` + 전원플랜 ‘깨우기 타이머 허용’. 완전 종료(전원 off) 시에는 다음 부팅 때 1회 따라잡음.
...
- **학회 채용게시판**: **한국대기환경학회**(kosae.or.kr:50010, 구인/구직) + **한국기상학회**(komes.or.kr:50000, 채용소식). 동일 `kunsolution/board.php` 엔진. 하이브레인·잡알리오에 없는 공고(국가미세먼지정보센터·국가농림기상센터·전력거래소 기상빅데이터·수자원공사 기상분석 등)를 직접 수집.
- **수도권대기환경청·지방환경청**(mcee.go.kr, 환경부 계열) + **산림청**(forest.go.kr, eGov 게시판) 직접 수집. 합격자·결과·일정 글 자동 제외. (산림청은 산림과학·산림환경 등 분야 매칭 시에만 통과 — 산림 노무직 제외)
- **극지연구소(KOPRI)** 직접 수집 — 기관 자체가 극지/대기/해양 분야라 분야검사 완화(노무·박사후·연수생 제외). 게시 35일 지난 공고는 마감 추정 제외.
- **수집 성능**: 9개 소스 + 잡알리오 상세를 **동시(병렬) 처리** → 약 15초. **교차 소스 중복 제거**(정규화 기관+제목).
- **상세 보강**: 하이브레인 D-N → 실제 마감일 환산, 학회는 상세페이지에서 마감일·자격 추출.
- **워크넷**(공공데이터포털 API, 선택): `WORKNET_KEY` 설정 시. 키 발급: https://www.data.go.kr → ‘한국고용정보원_워크넷 채용정보’.
- **나라일터**: best-effort(AJAX 동적 로딩이라 정적 파싱 0건).
- ⚠️ **환경부·국립환경과학원·산림청·국립산림과학원 누리집**: JS로 목록을 그리는 SPA라 **정적 스크랩 불가**(서버가 빈 셸만 반환). → 이들 기관 공고는 **학회 게시판·기상청·잡알리오**를 통해 수집됨(예: 국가미세먼지정보센터=환경과학원 산하 → 대기환경학회 게시판).
...
### 화면 기능
- **탭**: 전체 / ⭐ 추천 / 정부출연(과학기술) / 지자체·지방연구원 / 공공기관·공기업 / 연구기관·진흥원 / 대학·병원 / 하이브레인넷 / **🕒 최근 마감**.
- **🆕 NEW 배지**: 지난 실행(`data/seen.json`) 이후 처음 등장한 공고 표시 + ‘새 공고만’ 필터.
- **필터 바**: 새 공고 · 신입 지원가능 · 연봉 공개 · 고용형태 · 근무지.
- **🌙 다크 모드**: 토글(우상단), localStorage 저장.
- **🕒 최근 마감(5일)**: 최근 5일 이내 마감된 공고를 따로 모음(흐림·취소선 표시). 일반 탭에는 진행중 공고만 보임. 마감 0건이면 탭 숨김.
- 마감 상태는 `endDate` 기준 계산(`closed` 필드). 5일 초과 지난 공고는 수집에서 제외. 잡알리오는 진행중(ing=2) + 최근마감(ing=3) 2회 조회.
