...
#  - 매일 오전 8시 + 오후 5시에 run_daily.bat 실행 = 수집(scrape.py) + 메일발송(email_digest.py)
# ============================================================
...
$RunAt    = @('08:00','17:00')   # 매일 오전 8시 · 오후 5시
...
$triggers = @($RunAt | ForEach-Object { New-ScheduledTaskTrigger -Daily -At $_ })
...
Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $triggers -Settings $settings `
  -Description '기상·환경·대기·데이터 채용공고 매일 08시·17시 수집+메일발송' | Out-Null

Write-Host ("✅ 작업 등록 완료: '$TaskName' — 매일 " + ($RunAt -join ', ') + " 수집+메일발송") -ForegroundColor Green
